    ...
    <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-background border border-border rounded-lg shadow-lg w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 md:p-6 border-b border-border flex-shrink-0">
          <div>
            <h2 className="text-lg md:text-xl font-semibold text-foreground">{databaseName}</h2>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-4 md:p-6 space-y-4">
            ...
            {/* Mensaje de carga */}
            {isLoadingFlashcards && (
              <div className="flex flex-col items-center justify-center py-8 md:py-12 px-4 md:px-6 bg-muted/30 rounded-lg border border-border/50">
                <Loader2 className="w-6 md:w-8 h-6 md:h-8 animate-spin text-primary mb-3 md:mb-4" />
                <h4 className="text-base md:text-lg font-medium text-foreground mb-2">Cargando flashcards</h4>
            ...
            <div className={`space-y-4 md:space-y-6 ${isLoadingFlashcards ? 'opacity-50 pointer-events-none' : ''}`}>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 md:gap-4">
                ...
                <button
                  ...
                  className="group p-4 md:p-6 rounded-lg border border-border hover:border-primary/50 hover:bg-primary/5 transition-all text-left disabled:cursor-not-allowed"
                >
                  <div className="flex items-start gap-3 md:gap-4">
                    <div className="p-2 md:p-3 rounded-lg bg-primary/10 group-hover:bg-primary/20 transition-colors flex-shrink-0">
                      <Brain className="w-5 md:w-6 h-5 md:h-6 text-primary" />
                    </div>
                ...
                <button
                  ...
                  disabled={isLoadingFlashcards}
                  className="group p-4 md:p-6 rounded-lg border border-border hover:border-blue-500/50 hover:bg-blue-500/5 transition-all text-left disabled:cursor-not-allowed"
                >
                  <div className="flex items-start gap-3 md:gap-4">
                    <div className="p-2 md:p-3 rounded-lg bg-blue-500/10 group-hover:bg-blue-500/20 transition-colors flex-shrink-0">
              ...
              {/* Segunda fila - Modo Matching */}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 md:gap-4">
                <button
                  onClick={onStartMatchingMode}
                  disabled={isLoadingFlashcards}
                  className="group p-4 md:p-6 rounded-lg border border-border hover:border-green-500/50 hover:bg-green-500/5 transition-all text-left disabled:cursor-not-allowed"
                >
                  <div className="flex items-start gap-3 md:gap-4">
                    <div className="p-2 md:p-3 rounded-lg bg-green-500/10 group-hover:bg-green-500/20 transition-colors flex-shrink-0">
                      <Link className="w-5 md:w-6 h-5 md:h-6 text-green-500" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className="font-medium text-foreground mb-2 flex items-center gap-2">
                        Modo Matching
                        <Play className="w-3 md:w-4 h-3 md:h-4 text-green-500" />
                      </h4>
                      <p className="text-xs md:text-sm text-muted-foreground leading-relaxed mb-2 md:mb-3">
                ...
                <div className="hidden lg:block"></div> {/* Espacio vacío solo en pantallas grandes */}
              </div>
            </div>

            {/* Información adicional */}
            <div className="mt-4 md:mt-6 p-3 md:p-4 rounded-lg bg-muted/50 border border-border/50">
